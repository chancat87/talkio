            ...
            <div className="pb-4">
              {filtered.map((identity: Identity, idx: number) => (
                <IdentityItem
                  ...
                  onEdit={() =>
                    mobileNav ? mobileNav.pushIdentityEdit(identity.id) : setEditingId(identity.id)
                  }
                  onDelete={async () => {
...
export function IdentityEditPage({
  ...
  const handleSave = useCallback(
    (data: Omit<Identity, "id" | "createdAt">) => {
      if (existing) {
  ...
  return <IdentityForm identity={existing} onSave={handleSave} onClose={goBack} />;
}
...
function IdentityForm({
  ...
  const handleSave = useCallback(() => {
    ...
    const identityBoundBuiltInToolNames = selectedToolIds.filter(
      (toolName) => builtInEnabledByName[toolName] === false,
    );
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "var(--secondary)", paddingTop: "env(safe-area-inset-top, 0px)" }}
    >
      {/* iOS Navigation Bar */}
      <div
      ...
      >
        <button onClick={onClose} className="flex min-w-[60px] items-center px-2 active:opacity-60">
          <IoChevronBack size={22} color="var(--primary)" />
          <span className="text-[17px]" style={{ color: "var(--primary)" }}>
            {t("common.cancel")}
          </span>
        </button>
                  ...
                  {identitySelectableBuiltInTools.map((tool, idx) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          borderBottom:
                            idx < identitySelectableBuiltInTools.length - 1
                  ...
                  {mcpServers.map((srv, idx) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          borderBottom:
                            idx < mcpServers.length - 1 ? "0.5px solid var(--border)" : "none",
                        }}
                      >
                        <div className="mr-3 min-w-0 flex-1">
